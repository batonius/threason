...
TEST(valid_tokens) {
	struct {
		...
		{"\"\\\\\"", "\\\\", "", THSN_TOKEN_STRING},
		{"\"\\\\\\\"", "\\\\\\\"", "", THSN_TOKEN_UNCLOSED_ESCAPED_STRING},
		{"\"ABC\\\"\"\"", "ABC\\\"", "\"", THSN_TOKEN_ESCAPED_STRING},
		{"\t \"abc \\\"", "abc \\\"", "", THSN_TOKEN_UNCLOSED_ESCAPED_STRING},
	...
	for (size_t i = 0; i<sizeof(test_values)/sizeof(test_values[0]); ++i) {
		thsn_slice_t input_slice = THSN_SLICE_FROM_C_STR(test_values[i].input);
...
TEST(invalid_tokens) {
	char* invalid_tokens[] = {
		...
		"(", ")", "|", "~", "`", "'", ".", "*", "$",
		/* keywords */
		"nil", "True", "FALSE", "f", "n", "T",
		/* invalid numbers */
		"123.323.3", "123123e10.0", "-123e123e123", "0e-", "10e++8",
	};
	for (size_t i = 0; i<sizeof(invalid_tokens)/sizeof(invalid_tokens[0]); ++i) {
		thsn_slice_t input_slice = THSN_SLICE_FROM_C_STR(invalid_tokens[i]);
		thsn_slice_t token_slice;
		thsn_token_t token;
		ASSERT_EQ(thsn_next_token(&input_slice, &token_slice, &token),
