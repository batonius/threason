...
thsn_result_t thsn_vector_store_tagged_value(thsn_vector_t* vector,
    ...
    BAIL_ON_ERROR(
        thsn_vector_grow(vector, sizeof(tag) + value_slice.size));
...
thsn_result_t thsn_vector_store_null(thsn_vector_t* vector) {
    return thsn_vector_store_tagged_value(
        vector, THSN_TAG_MAKE(THSN_TAG_NULL, 0), THSN_SLICE_MAKE_EMPTY());
}

thsn_result_t thsn_vector_store_bool(thsn_vector_t* vector, bool value) {
    return thsn_vector_store_tagged_value(
        vector,
...
thsn_result_t thsn_vector_store_double(thsn_vector_t* vector, double value) {
    return thsn_vector_store_tagged_value(
        vector, THSN_TAG_MAKE(THSN_TAG_DOUBLE, THSN_TAG_SIZE_F64),
        THSN_SLICE_FROM_VAR(value));
}

thsn_result_t thsn_vector_store_int(thsn_vector_t* vector, long long value) {
    if (value == 0) {
        return thsn_vector_store_tagged_value(
...
thsn_result_t thsn_vector_store_string(thsn_vector_t* vector,
